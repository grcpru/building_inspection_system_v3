...
def generate_report_filename(
    ...
    Args:
        ...
        unit: Unit number (for single inspection)
        report_type: "single" or "multi"
    ...
    Examples:
        Single: "Argyle_Square_G210_2025-12-09"
        Multi: "Highett_Common_Building_Report_2025-12-09"
    """
    # Clean building name (remove special characters, replace spaces with underscores)
    if building_name:
        clean_building = building_name.replace(' ', '_').replace('(', '').replace(')', '').replace('&', 'and')
        # Limit length
        clean_building = clean_building[:50]
    else:
        clean_building = "Building_Report"
    ...
    if inspection_date:
        try:
            ...
            if isinstance(inspection_date, str):
                ...
                try:
                    date_obj = dt.strptime(inspection_date, '%Y-%m-%d')
                    date_str = date_obj.strftime('%Y-%m-%d')
                except:
                    date_str = inspection_date.replace('/', '-')[:10]
            else:
                date_str = str(inspection_date)[:10]
        except:
    ...
    # Build filename
    if report_type == "single" and unit and unit != 'Multiple Units':
        # Single unit inspection: "BuildingName_Unit_Date"
        clean_unit = unit.replace(' ', '_').replace('/', '-')
        filename = f"{clean_building}_{clean_unit}_{date_str}"
    else:
        # Multi-unit or building report: "BuildingName_Building_Report_Date"
        filename = f"{clean_building}_Building_Report_{date_str}"
...
if __name__ == "__main__":
    ...
    print("Combines template dashboard with API photo support")ls with units affected
- Photos embedded as thumbnails in data sheets
- Professional formatting and color coding
- Workflow Tracker with status monitoring
"""
